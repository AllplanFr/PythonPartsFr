# SlopedArchOpening | ALLPLAN FRANCE PythonParts Collection

`SlopedArchOpening` is a PythonPart for ALLPLAN that ...
...
`Office` → `ALLPLAN FRANCE` → `Sloped opening for architectural element`

## How to use it

1. Open the ALLPLAN Library.
2. Navigate to `Office` → `ALLPLAN FRANCE`.
3. Start `Sloped opening for architectural element`.
